...
- [ ] 在普通网页点击扩展图标会直接显示当前标签页内的隔离浮窗，不出现中间 popup，也不自动启动捕获。
- [ ] 浮窗空闲页可启动整页捕获、元素选择或打开捕获包；元素选择完成后自动回到浮窗的分析结果页。
- [ ] 捕获进行中隐藏并重新打开浮窗会恢复同一 session 及最新进度，不会重复启动或丢失任务。
- [ ] 浮窗在桌面和窄视口均不超出屏幕，内部溢出可滚动；浮窗之外不阻止页面交互。
- [ ] 元素选择期间浮窗不会遮挡点击目标，任务最小化后仍能看到阶段/进度信号并一键恢复。
- [ ] 浏览器限制页无法注入浮窗时会得到明确、非静默的反馈。
- [ ] 用户能够在执行采集前查看并修改已纳入 MVP 的资源与输出选项。
- [ ] 高级设置仅出现四项已确认能力，并将有效值传递到 adapter/bridge；Shadow DOM、trace 和内部调度参数不会出现在用户配置中。
- [ ] 分析完成后，用户能在开始资源处理前看到目标范围内的图片节点总数，并明确选择处理或跳过。
- [ ] 确认前资源集合变化会触发重新确认；仅引用节点数变化不会重复询问；执行期间进度总量不会因 DOM 动态变化而增加。
- [ ] 同一图片 URL 被多个节点复用时，分析结果会分别显示节点数与唯一资源数，且资源只处理一次。
- [ ] `srcset` 图片按浏览器实际选择的 `currentSrc` 计数和去重，普通 DOM 与开放 Shadow DOM 使用同一遍历语义。
- [ ] CSS 背景图片会显示独立的“不支持”数量和说明，但不会被误报为已处理资源。
- [ ] 处理图片时，UI 能按图片阶段显示至少 `已处理/总数` 与失败数；图片阶段结束后才进入后续转换阶段。
- [ ] 图片全部成功会自动继续；部分失败不会静默继续，并提供重试失败项、占位继续和取消三个动作。
- [ ] 重试后成功项不会发生重复网络请求或重复处理。
- [ ] 图片调度从不超过 4 个并发；单项 15 秒或阶段 60 秒到期后任务进入可诊断的失败恢复状态。
- [ ] 取消会终止排队和进行中的页面/后台请求，迟到结果无法更新或污染已取消或更新的 session。
- [ ] 图片累计达到 64 MiB 会进入可恢复的软限制提示；达到 128 MiB 后不会再分配新图片处理任务或继续扩大 artifact。
- [ ] 跳过图片时不会发起图片资源获取，且后续阶段仍能按约定策略完成。
- [ ] 跳过或失败的图片在结果中保留原几何与支持的框样式，且不会导致相邻 Auto Layout 项目重新排列。
- [ ] 已保存的用户配置会在约定的生命周期与作用域内恢复。
- [ ] 新标签页浮窗载入全局默认；同一标签页隐藏并恢复浮窗时保留临时修改和进行中 session。
- [ ] 临时捕获设置不会覆盖全局默认，点击“设为默认”后新会话才读取更新值。
- [ ] 刷新或关闭标签页会清除临时 session，且扩展存储中不会出现按站点资源清单或自动捕获历史。
- [ ] 启用剪贴板出口时，扩展会写入约定格式并明确反馈结果。
- [ ] 启用本地文件出口时，扩展会生成约定格式的可用文件并明确反馈结果。
- [ ] 同时启用两个出口时只执行一次页面捕获，并分别输出相同 payload。
- [ ] 一个出口失败不会撤销另一个已经成功的出口，界面会明确报告部分成功和失败原因。
- [ ] 长时间处理结束后不会自动触碰剪贴板或下载；最终输出点击会立即消费已准备 artifact，并获得新的用户激活。
- [ ] 输出失败后单独重试出口不会重新执行资源阶段或转换阶段。
- [ ] UI 不允许提交零出口配置；首次使用默认保持现有“复制到剪贴板”行为。
- [ ] 用户能够重新打开有效捕获包并把保存的结果再次复制到 Figma，回放结果与首次捕获的剪贴板 payload 一致。
- [ ] 捕获包包含可检测的格式版本；损坏或不支持版本的文件会被拒绝并给出明确错误，不会写入剪贴板。
- [ ] `.figit` 文件可作为 JSON 解析，且只包含一份 clipboard HTML payload；修改 payload 后 checksum 校验会失败。
- [ ] 捕获包 source URL 不包含 credentials、query 或 fragment，资源诊断不泄露完整远程 URL；同一资源仍可通过哈希标识关联。
- [ ] 回放有效 `.figit` 后写入剪贴板的 HTML 与首次捕获保存的 HTML 字节一致。
- [ ] 图片或字体资源失败时，行为符合约定的容错策略，且不会静默生成不可诊断的产物。
- [ ] 字体兼容模式会报告 exact/fallback 结果；快速模式不会发起页面或公共 CDN 字体请求；严格模式不会在存在不精确匹配时进入转换。
- [ ] 严格模式失败后可重试、切换兼容模式或取消，且三种路径都不会静默丢弃文字节点。
- [ ] UI 选项、扩展消息协议与底层转换能力具有自动化契约或行为测试覆盖。
- [ ] adapter 的阶段状态机与资源清单测试可以使用伪转换引擎运行，不需要加载 `@figit/dom-to-figma` 实现。
- [ ] adapter 不导入 `@figit/dom-to-figma` 的内部路径；具体上游集成集中在单一 bridge，替换或升级上游时无需修改 UI、配置存储、进度协议或捕获包模型。
- [ ] 若上游增加图片预处理 hook，未启用该 hook 时现有 `createFigmaConverter()` 行为和输出保持不变，并通过 changeset、包测试及干净安装兼容验证。
