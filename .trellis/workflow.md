...
Flow: `trellis-implement` -> `trellis-check` -> spec-update dispatch when needed -> commit (Phase 3.4) -> `/trellis:finish-work`.
...
Codex-only exact routes: Sol/medium is coordination-only and dispatches `trellis-research` at Terra/medium for missing evidence. Routine units use `trellis-implement` at Terra/high followed by `trellis-check` at Terra/max. Complex or cross-boundary units use explicit Sol/medium `hard_implementation` followed by Sol/xhigh `hard_checking`. Use the matching Sol assurance role for debugging, review, audit, or release gates. Claude Code retains its native `trellis-research`, `trellis-implement`, and `trellis-check` dispatch path; it does not require Codex model names or the four Codex assurance profiles.
...
most three such units concurrently. On Codex, dispatch complex or cross-boundary
units with the explicit `hard_implementation` (Sol/medium) route. Other
platforms use their native implementation agent without requiring a Codex route.
...
- **Codex route**: `bounded_worker` (`gpt-5.6-terra` / `high`); Claude Code uses its native `trellis-implement` agent without a Codex model requirement
...
Every completed implementation unit requires its own `trellis-check`
dispatch before the main session can integrate the result or proceed to finish.
On Codex, select `checking` (Terra/max) after routine implementation and
`hard_checking` (Sol/xhigh) after complex or cross-boundary implementation.
...
- **Codex route**: `checking` (`gpt-5.6-terra` / `max`) after routine work;
  `hard_checking` (`gpt-5.6-sol` / `xhigh`) after hard or cross-boundary work;
...
recording, but it must not edit the spec itself:
- Newly discovered patterns or conventions
- Pitfalls you hit
- New technical decisions

When an update is needed, dispatch `trellis-implement` with ownership limited
to the relevant `.trellis/spec/` paths and instruct it to use
`trellis-update-spec`. Even if the conclusion is "nothing to update", record
the judgment in the task evidence.
