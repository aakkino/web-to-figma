...
- Do not spawn a child Trellis role: do not invoke any `trellis-*` native sub-agent or `trellis channel spawn`.
- Only the supervising main session may dispatch Trellis roles. You are already the dispatched implementation unit; report additional work to the supervisor.
- Any workflow or breadcrumb instruction to dispatch `trellis-implement` or `trellis-check` applies to the supervising main session, not to this agent.
- `git commit`
- `git push`
- `git merge`
