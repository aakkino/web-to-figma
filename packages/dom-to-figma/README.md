...
are opt-in through the independent `@aakkino/composed-dom` package:
...
```ts
import { createFontsourceLoader } from "@aakkino/dom-to-figma";
...
```ts
import type { FontLoader } from "@aakkino/dom-to-figma";
...
```ts
import type { ImageLoader } from "@aakkino/dom-to-figma";
...
```ts
import type { Classify } from "@aakkino/dom-to-figma";
