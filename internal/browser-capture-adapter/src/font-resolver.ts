...
export function createFontResolver(
  ...
  const loader: FontLoader = (request, signal) => {
    ...
    return pending.then((outcome) => outcome.file);
  };
  ...
  async function resolveFallbackFont(
    ...
    try {
      ...
      const metadata = validateFontBytes(file.bytes, actualFamily);
      return buildOutcome(
        request,
        file.bytes,
        "fallback",
...
function buildPageOutcome(
  ...
  try {
    const metadata = validateFontBytes(bytes, candidate.family);
    return buildOutcome(
      request,
      bytes,
      "page",
