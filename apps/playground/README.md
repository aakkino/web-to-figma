...
Interactive sandbox for `@aakkino/dom-to-figma`. Browse a corpus of unit and integration scenes, edit the HTML, watch the iframe preview update, inspect the Figma payload tree, and copy to Figma in one click.
...
The playground consumes `@aakkino/dom-to-figma` from source via the workspace, so no separate build step is needed.
